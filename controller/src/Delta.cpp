...
class Delta{
    public:
        Delta(double rbody_,double rfoot_,double uppleglen_,double lowleglen_)
        {
            for(int i = 1;i <= 3;i++)
            {
                motor[i - 1].ID = i;
                motor[i - 1].pos_tar = 0.0;
                motor[i - 1].vel_tar = 0.0;
                motor[i - 1].tor_tar = 0.0;
            }
            leg.joint_data = motor;
            leg.joint_num = 3;
            leg.rbody = rbody_;//62.5
            leg.rfoot = rfoot_;//40.0
            leg.rdif = rbody_ - rfoot_;
            leg.uppleglen = uppleglen_;//110.0
            leg.lowleglen = lowleglen_;//250.0
        }
        void InverseKinematics();
        void ForwardKinematicsP();
        void ForwardKinematicsVF();
        void CalJacob();
        void CalreJacob();
        void Statics();
...
void Delta::ForwardKinematicsVF()
{
    for(int i = 0;i < 3;i++)
    {
        leg.endv[i] = 0.0;
...
extern "C"{
    Delta* delta_new(double rbody_,double rfoot_,
                    double uppleglen_,double lowleglen_){
        return new Delta(rbody_,rfoot_,uppleglen_,lowleglen_);
    }
    /* write */
    void set_endp_tar(Delta* obj,double endp,int axis){
        obj->leg.endp_tar[axis] = endp;
        return;
    }
    void set_motor_pos(Delta* obj,double pos,int id){
        obj->motor[id].pos = pos;
        return;
    }
    void set_motor_vel(Delta* obj,double vel,int id){
        obj->motor[id].vel = vel;
        return;
    }
    void set_endf_tar(Delta* obj,double endf,int axis){
        obj->leg.endf_tar[axis] = endf;
        return;
    }
    /* read */
    double get_endp(Delta* obj,int axis){
        return obj->leg.endp[axis];
    }
    double get_endv(Delta* obj,int axis){
        return obj->leg.endv[axis];
    }
    double get_motor_pos_tar(Delta* obj,int id){
        return obj->motor[id].pos_tar;
    }
    double get_motor_tor_tar(Delta* obj,int id){
        return obj->motor[id].tor_tar;
    }
    /* calculate */
    void inverse_kinematics(Delta* obj){
        obj->InverseKinematics();
        return;
    }
    void forward_vf(Delta* obj){
        obj->ForwardKinematicsVF();
        return;
    } 
    void cal_jacob(Delta* obj){
        obj->CalJacob();
        return;
    }
    void statics(Delta* obj){
        obj->Statics();
        return;
    }
}